...
exports.getSenderLid = exports.toJid = void 0;
...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('botForwardedMessage' in message) {
        m.botForwardedMessage = message.botForwardedMessage;
    }
    ...
    if ('viewOnce' in message && !!message.viewOnce && !m.viewOnceMessage) {
...
const getContentType = (content) => {
    if (content) {
        ...
        const key = keys.find(k => (k === 'conversation' || k.includes('Message') || k === 'botForwardedMessage') && k !== 'senderKeyDistributionMessage');
        return key;
    }
};
